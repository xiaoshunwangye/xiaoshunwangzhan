...
class MorphEngine {
  ...
  private raf: number;
  private boundContextLost: () => void;
  private boundLoop: () => void;
  private dprCap: number;

  constructor(container: HTMLElement, options: MorphEngineOptions) {
    ...
    this.boundContextLost = this.onContextLost.bind(this) as unknown as () => void;
    ...
    this.boundLoop = this.loop.bind(this) as unknown as () => void;
  ...
  loadTextures() {
    this.items.forEach((item, index) => {
      ...
      img.src = item.image;
      img.onload = () => {
        // 将图片绘制到 canvas 以确保 SVG 等格式也能正确加载到 WebGL 纹理
        // 按 dprCap 放大绘制，避免高清屏 / 放大显示时 SVG/图片发虚
        const dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, this.dprCap));
        const canvas = document.createElement('canvas');
        const naturalW = img.naturalWidth || 800;
        const naturalH = img.naturalHeight || 450;
        const w = naturalW * dpr;
        const h = naturalH * dpr;
        canvas.width = w;
        canvas.height = h;
        ...
        this.sizes[index] = [naturalW, naturalH];
